...
propose_tree <- function(df, x, min_node, max_attempt = 10, i, probit = FALSE, miss_row = NA, cat_list = NA) {
  ...
  while(isFALSE(decent_tree) && attempt <= max_attempt) {
             ...
             # if(nrow(df) != 1) {
               ...
               while(bad_grow_variable) {
                 ...
                 if(is.numeric(x[,grow_variable])) {
                    ...
                    # new_point <- sample(split_set,sample(seq_along(split_set), 1)) #ifelse(length(split_set) == 1, NA, sample(split_set, 1)) #setequal(unique(x[,grow_variable]), unique(df$split_value[df$split_variable == grow_variable]))
                 }
                 if(!is.na(new_point)) { bad_grow_variable <- FALSE }
               } #End while(bad_grow_variable)
             # } else { #This is if nrow(df) == 1
             #   new_point <- ifelse(is.numeric(x[,grow_variable]), sample(unique(x[,grow_variable]), 1), sample(unique(x[,grow_variable]), 1))
             # }
    ...
    if(all(probit, decent_tree)) decent_tree <- !Reduce(any, lapply(split(miss_row, change_points), all))
