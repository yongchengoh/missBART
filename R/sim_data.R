...
#' \item{y}{responses}
#' \item{x}{covariates}
#' \item{ome}{precision matrix}
#' \item{mean}{mean vector}
#' \item{p}{number of responses}
...
sim_data_friedman <- function(n, p = 1, scale_par = 1, omega_diag = 1, Omega = NULL) {
  ...
  X <- matrix(stats::runif(5 * n), nrow = n, ncol = 5)
...
sim_data_trees <- function(n, p, q, min_x = 0, max_x = 1, trees = 1, ome = NULL, kappa = NULL, splits = NULL, ...) {
  ...
  if(is.null(ome)) ome <- stats::rWishart(1, p + 1, diag(p))[,,1]
  # ome <- ifelse(is.null(ome), stats::rWishart(1, p + 1, diag(p))[,,1], ome)  #stats::rWishart(1, p + 1, diag(p))[,,1]
...
#' @return A list with the following components
#' \describe{
#' \item{B}{probit coefficients}
#' \item{m}{missingness indicators}
#' \item{missing_y}{inputted y with missing values deleted}
#' \item{missing_prop}{proportion of missingness per response variable}
#' \item{missing_index}{indices of missing cells in \code{missing_y}}
#' \item{obs_index}{indices of observed cells in \code{missing_y}}
#' \item{corR}{correlation matrix}
#' \item{zmod}{probit latent variable}
#' }
#' @export
#'
#' @examples
